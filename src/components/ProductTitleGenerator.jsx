...
const { FiShoppingBag, FiSettings, FiRefreshCw, FiCheck, FiX, FiEdit3, FiCpu, FiTag, FiFileText } = FiIcons;
...
const ProductTitleGenerator = ({ onGenerateContent, isGenerating, hasDescription, apiKey }) => {
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        setProductParams(prev => ({ ...prev, ...settings }));
  ...
  }, [productParams, showAdvanced, showManualPrompt, showModelSelector, selectedModel, contentType]);
  ...
  const paramOptions = {
    ...
    platform: [
      'General', 'Taobao', 'Tmall', 'JD.com', 'Pinduoduo', 'Amazon', 'eBay',
      'Shopify', 'WeChat Store', 'Douyin Shop', 'Xiaohongshu'
    ],
    tone: [
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-gray-900">电商产品内容生成器</h3>
        </div>
      ...
      <div className="mb-4 p-3 rounded-lg bg-gray-50">
        <div className="flex items-center space-x-2">
          ...
          <span className={`text-sm font-medium ${hasDescription ? 'text-green-700' : 'text-red-700'}`}>
            {hasDescription ? '图像已分析 - 可以生成产品内容' : '请先分析产品图像'}
          </span>
        </div>
        {selectedModel && (
      ...
      <div className="space-y-4">
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="输入产品的关键特性、卖点、优势等信息..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="输入特殊要求，如：'强调性价比'、'突出品质感'、'面向年轻消费者'等..."
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                  rows="3"
                />
                <p className="text-xs text-gray-500 mt-1">
                  这将指导 AI 根据你的具体要求生成产品标题和描述。
                </p>
              </div>
              ...
              {(productParams.keyFeatures?.trim() || productParams.manualPrompt?.trim()) && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <h5 className="text-sm font-medium text-blue-900 mb-2">自定义信息预览:</h5>
                  ...
                  {productParams.manualPrompt?.trim() && (
                    <p className="text-sm text-blue-700">
                      <strong>生成指令:</strong> {productParams.manualPrompt}
                    </p>
                  )}
                </div>
              )}
        ...
        >
          {isGenerating ? (
            ...
            <>
              ...
              <span>
                生成{contentType === 'title' ? '产品标题' : contentType === 'description' ? '产品描述' : '产品标题和描述'}
              </span>
        ...
        {!hasDescription && (
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-700">
              💡 <strong>提示:</strong> 请先上传产品图像并进行分析，然后基于图像内容生成产品标题和描述。
            </p>
          </div>
        )}
        ...
        {hasDescription && (
          <div className="mt-4 p-3 bg-green-50 rounded-lg">
            <p className="text-sm text-green-700">
              ✅ <strong>就绪:</strong> 点击上方按钮生成专业的电商产品{contentType === 'title' ? '标题' : contentType === 'description' ? '描述' : '标题和描述'}。
            </p>
            {(productParams.keyFeatures?.trim() || productParams.manualPrompt?.trim()) && (
              <p className="text-sm text-green-700 mt-1">
                🎯 <strong>自定义信息:</strong> 将根据你提供的产品特性和要求进行生成。
              </p>
            )}
            {selectedModel && (
              <p className="text-sm text-green-700 mt-1">
                🤖 <strong>AI 模型:</strong> 使用 {selectedModel} 进行高质量内容生成。
              </p>
            )}
          </div>
        )}
